...
async def _unhandled(request: Request, exc: Exception):
    ...
    return JSONResponse(status_code=500, content={"status": "error", "error": "internal_server_error"})
...
async def ingest(request: Request):
    ...
    if isinstance(body, list):
        ...
        return JSONResponse(status_code=400, content={"status": "error", "error": "expected {events:[...]} or a bare list"})
