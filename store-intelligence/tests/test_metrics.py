...
def _zone(track_id, zone="PURPLLE_BLR_1008_Z_Z01", name="Shelf A",
...
def _zone_exit(track_id, zone="PURPLLE_BLR_1008_Z_Z01",
               ts="2026-04-10T10:05:00"):
...
def test_dwell_computed(tmp_db):
    _ingest(tmp_db, [
        _zone(5001, ts="2026-04-10T10:00:00"),
        _zone_exit(5001, ts="2026-04-10T10:03:00"),
        _entry("V1"),
    ])
    r = get_metrics("ST1008", db_path=tmp_db)
...
def test_heatmap_normalized(tmp_db):
    ...
    events = (
        [_zone(5000 + i, zone=z1, name="A") for i in range(10)] +
