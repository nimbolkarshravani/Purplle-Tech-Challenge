...
def _check_visitor_spike(entries: list, now: datetime, baseline: str) -> list:
    ...
    if avg > 0 and current_count > SPIKE_MULTIPLIER * avg:
        anomalies.append({
            ...
            "severity": "warning",
            "detail": (
                f"Current hour has {current_count} entries "
                f"({SPIKE_MULTIPLIER}× rolling avg {avg:.1f})"
            ),
            "detected_at": now.isoformat(),
        })
    return anomalies
...
def _check_abandonment_surge(queue_evts: list, now: datetime) -> list:
    ...
    if rate > ABANDONMENT_RATE_THRESHOLD:
        return [{
            "type": "QUEUE_ABANDONMENT_SURGE",
            "severity": "critical",
            "detail": (
                f"Abandonment rate {rate:.0%} in last {ABANDONMENT_WINDOW_MINUTES} min "
                f"({abandoned}/{len(recent)} events)"
            ),
            "detected_at": now.isoformat(),
        }]
    return []
...
def _check_dead_zones(zone_evts: list, entries: list, now: datetime) -> list:
    ...
    dead = [
        {"zone_id": zid, "zone_name": name}
        for zid, name in all_zones.items()
    ...
    return [{
        ...
        "severity": "info",
        "detail": (
            f"{len(dead)} zone(s) had 0 visits in last {DEAD_ZONE_WINDOW_HOURS}h "
            f"while store has active traffic: "
            + ", ".join(z["zone_name"] for z in dead[:5])
            + ("..." if len(dead) > 5 else "")
        ),
        "zones": dead,
        "detected_at": now.isoformat(),
